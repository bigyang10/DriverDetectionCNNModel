...
# Generate Class Count Bar Chart
def create_class_count_chart(class_counts):
    fig, ax = plt.subplots()
    ax.bar(class_counts.keys(), class_counts.values(), color='orange')
    ax.set_xlabel("Class Name")
    ax.set_ylabel("Number of Predictions")
    ax.set_title("Number of Predictions per Class")
    ax.set_xticklabels(class_counts.keys(), rotation=45, ha='right', fontsize=9)
    return fig

# Initialize session state for history and class counts
if "history" not in st.session_state:
    st.session_state.history = []
if "class_counts" not in st.session_state:
    st.session_state.class_counts = {label: 0 for label in CLASS_LABELS.values()}

st.title("TFLite Image Classifier with History and Class Count Chart")
...
if uploaded_file is not None:
    ...
    st.image(image, caption="Uploaded Image", use_container_width=True)  # Use use_container_width to fix warning
    ...
    # Update class count
    st.session_state.class_counts[predicted_label] += 1
...
if "show_class_count" not in st.session_state:
    st.session_state.show_class_count = False

# Toggle button for Class Count Chart
if st.button("Toggle Class Count Chart"):
    st.session_state.show_class_count = not st.session_state.show_class_count

# Conditionally display Class Count Chart
if st.session_state.show_class_count:
    st.subheader("Class Count Chart")
    class_count_fig = create_class_count_chart(st.session_state.class_counts)
    st.pyplot(class_count_fig)
...
if st.session_state.history:
    ...
    for i, entry in enumerate(reversed(st.session_state.history), start=1):
        if entry["confidence_score"] >= confidence_threshold:
            ...
            st.image(entry["image"], caption="Uploaded Image", use_container_width=True)
