...
# Grad-CAM Function (Simulated for TFLite Models)
def apply_gradcam(image):
    """Simulated Grad-CAM output (since TFLite does not support gradients)."""
    heatmap = np.random.rand(image.size[1], image.size[0])  # Simulated heatmap
    heatmap = cv2.normalize(heatmap, None, 0, 255, cv2.NORM_MINMAX)  # Normalize heatmap
    heatmap = np.uint8(heatmap)  # Convert to uint8
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)  # Apply color map
    overlay = cv2.addWeighted(np.array(image), 0.6, heatmap, 0.4, 0)  # Blend with image
    return overlay

# Streamlit UI
st.title("TFLite Image Classifier with Grad-CAM")
...
if uploaded_file:
    ...
    ax.set_xticklabels(CLASS_LABELS.values(), rotation=45, ha='right', fontsize=9)  # Rotate class names
    st.pyplot(fig)

    # Apply Grad-CAM Visualization (Simulated for TFLite Models)
    st.subheader("Grad-CAM Visualization")
    gradcam_image = apply_gradcam(image)
